...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Air Quality');
}
...
async function getAirQuality(lat: number, lon: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/air-quality?${params}`);
  if (!res.ok) throw await httpError(res, 'Air Quality API error');
